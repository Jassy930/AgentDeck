...
## Relay Web Test Companion W0–W3 automatic 门禁（2026-07-30）
...
W3.5 网络故障、W3.6 三次 fresh run与 W3.7 双路终审均已完成；Web automatic scope complete。
...
主 tab 在 paired revision `1`/Stable 持有 profile Web Lock 时，第二 tab 必须 acquire=false，并以
`web.remote.writer_generation_missing` 在 load/mutation/send 前拒绝；交接后新 tab取得锁并发布
...
三轮都必须通过 runner-owned TCP 透明代理，Relay TLS/SPKI 端到端不终止，proxy evidence 必须
`parsedProtocol=false`。disconnect 在 armed connection 累计 2 KiB 后切断并由 bounded reconnect 恢复；
...
2026-07-30 最终 code/test candidate `d4446ef1e26945458406e6811804a68fa617b7fd` / tree
`2bb877b37c160d9c35f0f54a835b9451a764cc38` 连续三轮 fresh exit 0。最终 terminal 读回 W2b `3`、W3 detail
`18`、W3 aggregate `6`、`allRunsConsistent=true`；每轮 command/completed 与 approval total/applied 各
`1/1`，recovery revoke `1`、active grant `0`，plaintext 与 cleanup 全部 absent。该候选同时通过 `--all`、
contention、browser kill、network fault、完整 Rust/Swift/iOS/P4/P5、release allocator 与静态门禁。
完整 `p5` 首次在冷 Simulator 出现一次 UI observation timeout，runner 完整 cleanup；同候选 focused lifecycle
和完整 aggregate 随后均 PASS，因此不把瞬时失败冒充最终证据，也不把它静默删除。

W3.7 对 `1a74cf7..d4446ef` 整个增量完成 `spec/security` 与 `quality/Git` 双路终审，两路均 Approved，
P0/P1/P2=0。Relay/Runtime/E2EE owner/schema 未变，TypeScript ownership、runner 正反例、plaintext、
network/no-net、docs/diff/status 与 cleanup 均通过；当前状态为 `Implemented (Web automatic test scope)`。

W4 的公网、物理设备、production SPKI/signing、第二台 Mac 与真实 vendor继续 BLOCKED，且不计 automatic PASS。
...
composition/pairing lifecycle 与 P5.7 macOS SessionSource registry automatic Task 已完成。上述 Task
收口时 P5/P6 进度为 7/9、0/4；后续 rescue P5.8-lite、P5.9 与 P5 automatic Phase Exit 已完成，当前 P5
automatic scope 为 9/9，P6 保持 0/4 且不进入当前 MVP。
...
P5.8–P5.9 与 P5 Phase Exit 在 P5.4 收口时仍是 automatic 必做项；后续 rescue P5.8-lite 已完成，当前只剩
P5.9 与 Phase Exit。真实公网 WSS、production-signed Keychain、
...
post-MVP `BLOCKED`。后续 P5.7 与 rescue P5.8-lite、P5.9、P5 automatic Phase Exit 已独立收口；当前 P5
automatic scope 为 9/9。
...
候选上的结论均为 `P0=0 / P1=0 / P2=0`。P5.7 已独立收口，当时 P5 为 7/9。后续 rescue R3/R4 已完成
P5.8-lite、P5.9 fixed-topology Simulator Relay E2E 与 P5 automatic Phase Exit。旧 remote
macOS picker/pairing/receipt UI、真实公网 WSS、production-signed Keychain、物理 iPhone、第二台 Mac、真实
Codex/Claude Code vendor 与 destructive purge 全部 post-MVP `BLOCKED`，不计 PASS。

## Relay Companion MVP rescue R3 本机 pending-device 控制面

R3 只增加被控 Mac 的本机 pending-device 确认入口，不增加 machine picker、remote pairing sheet、远程
macOS 控制面、协议版本或第二套运行平台。production composition 暴露已有 local source 的 typed
`LocalPairingAdministration`；Preview/fixture 保持 `nil`。controller 绑定 pairing ID、request hash、完整
DeviceSign fingerprint 与 expiry，confirm/cancel 按 ID single-flight，terminal receipt 到 stream 删行之间继续
禁用。AlreadyHandled 保留 winner/state；同 ID 身份变化、迟到 completion 与 receipt ID 不一致全部 fail-close。

冻结 code/test manifest 共 5 paths，按 `blob <git hash-object> <path>` C-locale 排序后的 SHA-256 为
`9c3bd63c9e56ef244d0d72da3192cfe3ade31b80c7a41675ee804355b56720ba`：

```text
Sources/AgentDeck/AppDelegate.swift
Sources/AgentDeck/Machines/PendingDeviceApprovalController.swift
Sources/AgentDeck/Preview/PreviewBootstrap.swift
Sources/AgentDeck/SessionSources/AppSessionSourceComposition.swift
Tests/AgentDeckTests/PendingDeviceApprovalTests.swift
```
...
xcrun swift-format lint --strict \
  ...
  Tests/AgentDeckTests/PendingDeviceApprovalTests.swift
xcrun swift-format lint --strict \
  --configuration '{"indentation":{"spaces":4},"lineLength":200}' \
  Sources/AgentDeck/AppDelegate.swift
./script/build_and_run.sh --verify
scripts/verify-agent-docs.sh
git diff --check
...
**Automatic/readback 证据（2026-07-28）：** focused `46/46`；fresh 顶层 warnings-as-errors
`1161 XCTest / 4 skipped / 0 failure + 48 Swift Testing / 0 failure`；ownership、两组 changed-file strict
format、docs 与 diff 均 exit 0。stable selfcheck 精确返回 `daemon.client.socket_missing`，这是当前账号未运行
canonical daemon 的环境 `BLOCKED`，不计 PASS；hermetic local-runtime smoke 启动真实 ephemeral/no-remote
daemon，以同一 UDS 完成 Rust + Swift selfcheck、双 installation、receipt/replay/stream 与 cleanup，exit 0。

真实 `dist/AgentDeck.app` 经 bundle/helper/resource/minos verify 后打开 production 菜单和“本机配对请求…”面板；
当前 stable UDS 缺失时可见读回“本机 Runtime 暂不可用，可以稍后重试”，没有误显示为空列表或授权成功。
fixture AppKit tests 读回完整 32-byte fingerprint、confirm/cancel、AlreadyHandled、expired/canceled/security
failure，且页面不含 request hash、secret、grant 或 key material。退出后 exact bundle process absent，`dist/`
...
## Relay Companion MVP rescue R4.8/R4.9 重复性与 review

> 历史证据，已失效：R5.2 首轮完整 `p5` 暴露 failure cleanup 遗留本轮 `agentdeckd --exec-gate`
> 后代。下列结论只属于旧 candidate `3fb83e8`，不得继续作为当前 R4/R5 完成证据。

R4.8/R4.9 唯一 candidate 为 commit `3fb83e83a34abd659e32b817cf1cabccf999ca79`、tree
`d136544a47f64ffabe0e60a464cc7f8cd18f5f33`，相对本地 `master` 为 `0 behind / 285 ahead`。在不修改
tracked/untracked 文件、不切换 HEAD 且不复用前轮私有 root 的前提下，连续三次 fresh lifecycle E2E 均于 daemon
generation 2 完成 command `1/1`、approval `1/1`、revoke `1`，最终 active grant `0`；三轮 Relay plaintext
扫描均 absent，host PID/root/invite/socket/owned Simulator cleanup 均为 true。

同一 candidate 上 `bash scripts/verify-relay-companion-mvp.sh p4-auto` 与完整 `p5` 均 PASS；SessionSource
`25/25`，RelayClient `457 executed / 4 external entitlement skipped / 0 failed`。完整证据为 iOS `134/134`、Swift
`1169 XCTest / 4 external entitlement skipped + 48 Swift Testing / 0 failed`、Rust daemon lib
`1711 passed / 3 ignored / 0 failed` 且 workspace 其余 integration/doctest 零失败。daemon network/no-net、协议
ownership 正反例、四 schema parity、external BLOCKED contract、Swift/shell format、`bash -n`、agent docs 与
`git diff --check` 全部通过；没有协议/schema/version 漂移、第二套 runtime platform、co-author、secret、DB、证书
或生成物进入 Git。

`spec/security` 与 `quality/Git` 在该 candidate 上均 Approved，`P0/P1/P2=0`。物理 iPhone 和第二台 Mac 继续
精确为 exit 78、versioned `BLOCKED`、`mutations=0`、`evidence=[]`、`summaryGenerated=false`，不计 PASS。
cleanup 先以 `git check-ignore -v` 读回 `ios/AgentDeckMobile.xcodeproj/` 的 ignore owner，再将生成工程移出
worktree；`.build`/`target` 缓存未删除。最终无本 worktree Relay/daemon、无 booted Simulator，candidate HEAD/tree
未漂移且 status clean。R4、P5.9 与 P5 automatic Phase Exit complete；最终 MVP closeout 仍须执行 R5。

## Relay Companion MVP rescue R5.1 closeout candidate

R5.1 只把上述 R4 证据同步到 rescue implementation 与本质量记录，并用包含两份文档的 docs-only scoped commit
生成 closeout candidate；不修改 production、test、runner、协议、schema 或版本。R4 evidence candidate 继续固定为
`3fb83e83a34abd659e32b817cf1cabccf999ca79` / tree `d136544a47f64ffabe0e60a464cc7f8cd18f5f33`。
R5.2 必须从 R5.1 提交后的 clean 状态重新冻结最终 commit/tree/path hashes，并在同一新 candidate 上执行全量
Rust/Swift/iOS/P4/P5 门禁；任何后续仓库修改都会使其失效并回到 R5.1。外部槽位继续保持 BLOCKED。

## Relay Companion MVP rescue R4.9 failure-path 重开

R5.2 在 closeout candidate `5966c9867848e4ea176d61eeeb296e0c2fb280c3` 上首轮 Rust 全量、Swift全量、
iOS `134/134` 与 `p4-auto` 均 exit 0；完整 `p5` 已完成 pairing/local approve/business-ready，但 UI 未在
60 秒内观察到 `synthetic Codex response`，`xcodebuild` exit 65。failure trap 返回后仍存在本轮
`agentdeckd --exec-gate` PID `54930`、PPID `1`；经精确命令与启动时间核对后 TERM→KILL，最终 absent。
因此旧 `3fb83e8` 的 R4.9/P5 Phase Exit、`5966c98` 的 R5.1 candidate 与同轮可拼接门禁全部失效。

replacement implementation commit `5bd57ba33d03abbf7a4fcb7fa2e427a30d329bc2` / tree
`37b7bbe240e601dcc7654349b97318c9e0bd28d5` 在停止 cargo 或 host 前递归捕获本轮 PID + `ps lstart` 身份树，
TERM 后只 KILL 身份仍匹配的捕获后代并等待 absent/zombie；不按进程名全局清理。动态 contract 分别构造
cargo-root 与 host-root 的 TERM-resistant reparent 子进程，均被真实回收。最终 `bash -n`、contract、
`git diff --check` exit 0，测试后无 probe/Relay/daemon/exec-gate 残留；两路径 manifest SHA-256 为
`fd5a6c5e93c1391f4536d5fd9b4746e859c1ad7ca660dbbcb58f60026de9bd61`。

首版修复 WIP 的 fresh lifecycle 与完整 `p5` 已 PASS，但 final implementation 随后补齐 host-root fallback，
...
## Relay Companion MVP rescue R4.8/R4.9 restart handoff committed candidate 收口

唯一 evidence candidate 为 `8cfdc0d0bfd74186dafaf09da4333f97e10803a2`，tree 为
`dca2aa3c1a5da8c08bdd9f2b1eff83c825f754e9`，相对本地 `master` 为 `0 behind / 298 ahead`。三轮
lifecycle、aggregate、review 与 cleanup 全程未修改仓库或切换 HEAD；5-path implementation manifest 复算仍为
`7beefc9623398f2ef8527f2bbde624e11ed85332a39b504d1d4e8b23b4ecf185`。
...
host/root/invite/socket/owned Simulator cleanup 全部 absent。同一 candidate 的完整 `p5` exit 0：SessionSource
`25/25`，RelayClient `457 executed / 4 external entitlement skipped / 0 failed`；`p4-auto` exit 0：machine E2E
`2 passed / 1 interactive ignored`，persistent CLI、trust reset、Relay client、协议契约、四 schema、daemon
network/no-net 与文档门禁全部通过。物理 iPhone 与第二台 Mac 继续按 versioned contract `BLOCKED`，不计 PASS。

同一 hash 的最终 `spec/security` 与 `quality/Git` review 均为 Approved，`P0/P1/P2=0`。前者确认 final Close
只在 Relay Core disconnect 与 dependent-device cleanup 后返回，同进程 reconnect 保持原路径；Active startup
reservation 只开放既有 512 条 / 16 MiB 有界传输队列，不开放 Runtime，Dormant/Retire、未领取 lane、
reservation drop 与 rollback 继续 fail-close；协议/schema/storage/version 和 vendor-token 边界均未改变。后者确认
健康 Close、device generation cut、reservation 首帧/drop 与 manager rollback 均有确定性回归；candidate 只比失效的
`cfa90b6` 增加 5 个 code/test 路径和 2 个事实源文档，无冲突标记、二进制、DB、日志、secret 或运行数据进入 Git。

ignored 的 `ios/AgentDeckMobile.xcodeproj/` 与 `ios/AgentDeckMobile/Info.plist` 已移入本机可恢复废纸篓；没有删除
`.build`、`target`、`dist`。最终读回无 Relay/daemon/runner/xcodebuild 进程、无 Booted Simulator，HEAD/tree
未漂移且 `git status --short --branch` clean。R4、P5.9 与 P5 automatic Phase Exit complete；最终 MVP 仍必须在
新的 R5 closeout candidate 上从零执行完整矩阵，不能拼接本节证据。

## Relay Companion MVP rescue R5.2 dual-scope readiness 反例与 R4/R5 重开

R5 closeout candidate `2ddba9ba54820c37fce7efe698cd58d869be2314` / tree
`1f7e95f5d7543bebdd9e7f27513abc48cb9c0c8c` 的完整 Rust workspace 已通过：daemon
`1719 passed / 3 ignored / 0 failed`，其余 integration/doctest 零失败。随后完整
`RUSTC_WRAPPER= swift test -Xswiftc -warnings-as-errors` 在 `MachineScopeRealIntegrationTests` 稳定失败；focused
重跑仍复现 host 30 秒内无法满足硬编码 `BusinessReady`。因此 `2ddba9b` 与旧 R4 evidence `8cfdc0d` 均已失效，
上述 Rust 结果和旧三次 lifecycle/aggregate/review 只能保留为诊断证据，不能拼接到当前 candidate。

反例来自测试 host 把两种合法拓扑压成同一个资源计数。单远端/Simulator 必须精确读回 writer/live/job
`2/2/2`；macOS dual-scope 还由 `SessionSourceRegistry` 持有本机 source，稳定形态为 writer 精确 `3`、live/job
相等且为 `3` 或 `4`。两者都必须保持 barrier/snapshot sender `0/0`。Swift 仍独立读回 remote catalog 和
conversation connected，host 仍验证 transition complete、active grant、semantic ACK 与资源残留；该修复没有以
放松 business-ready 条件换取 PASS。

replacement implementation commit 为 `6dc2822942a418e144d38735a0a51892cf7a885d`，tree 为
`b887255f2a45d44f0eee1b2cf5b2e13b071828cc`，相对本地 `master` 为 `0 behind / 300 ahead`。冻结 2-path
code/test manifest：

```bash
dual_scope_readiness_manifest=(
  agentdeckd/tests/relay_v2_machine_e2e.rs
  Tests/AgentDeckTests/MachineScopeRealIntegrationTests.swift
)
test "${#dual_scope_readiness_manifest[@]}" -eq 2
for manifest_item in "${dual_scope_readiness_manifest[@]}"; do
  test -f "$manifest_item" || exit 1
  printf 'blob %s %s\n' "$(git hash-object "$manifest_item")" "$manifest_item"
done | LC_ALL=C sort | shasum -a 256
```

SHA-256 固定为 `e8a8ea64101bb509dd55a61005296a48004e34cd12894a19403cc2e1a67993ec`。fresh
implementation gates：

- `relay_v2_machine_e2e` 为 `3 passed / 1 interactive ignored / 0 failed`；
- `MachineScopeRealIntegrationTests` focused 连续 `3/3 PASS`；
- 完整 Swift 为 `1169 XCTest / 4 external entitlement skipped / 0 failure + 48 Swift Testing PASS`；
- `cargo fmt --all -- --check`、Swift strict format、`git diff --check` 与 scoped Clippy 均 PASS；直接全局
  `-D warnings` 只命中三项既有 Codex adapter lint，按本页冻结 allow 列表重跑通过，没有修改无关代码。

本切片没有修改 production、协议、schema、storage 或版本，也没有增加新的本地运行平台。本节与 rescue
implementation 计划通过 docs-only scoped commit 生成第七次 replacement R4.7 candidate。提交后所有 phase 证据从零
计数：连续三次 fresh lifecycle、完整 `p5`、`p4-auto`、必要的完整 Rust/Swift/iOS 与静态门禁、`spec/security`、
