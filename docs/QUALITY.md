...
- [ ] 侧栏渲染出品牌行、新建会话与搜索框、按日期分组的会话列表和本机 Agent 状态四段结构；
      底部每个 agent 只占一行，兼容性警告以 ⚠ 标出、详情在悬停提示中。
...
- [ ] 搜索框可键入中文，按标题或项目名实时过滤；点击 agent 卡片或侧栏 agent 行只看该 agent，
      再点取消；无结果显示“没有匹配的会话”。空态卡片与侧栏 agent 行计数一致。
...
- [ ] Tab 聚焦虚拟会话列表后，上下键能越过可见区并滚到目标，Return 打开该会话；
      Tab / Shift-Tab 能离开列表，搜索框中的方向键不触发会话导航。
