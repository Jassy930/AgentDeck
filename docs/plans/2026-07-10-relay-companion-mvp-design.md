...
P0/P1/P2=0。P3.1 provisioned signed Keychain/LaunchAgent roundtrip、真实 vendor、物理设备、第二台 Mac、
...
- 设备 replay/cursor/quarantine 等非 counter 状态更新也必须先把 previous Stable 与 exact next full-state
  commitment 写入 Keychain `statePending`。重启只允许 previous 回滚或 exact next finalize；任何同 scope、
  revision +1 但 commitment 不同的 authenticated sibling 都是 fork，必须 quarantine + retire。
...
macOS executable 侧增加 `LocalDaemonSessionSource`（current RuntimeEnvelope v5 over UDS）与
...
Runtime v5 的 commandless Error 只是 conversation diagnostic，不结束 observation/active turn；带 commandID 的
Error 只允许 fixed `daemon.runtime.execution_failed / agent execution failed / null` 并作为 Failed terminal。
Relay/fixture inbox 只能从后一种 terminal Error 派生 failed item。

broadcast 不能使用默认无界缓冲：catalog/machine/session resource state 使用 `.bufferingNewest(1)`，conversation event channel 固定最多 512 条。`yield` 返回 dropped 时 source 必须发出 typed `lagged` 状态并终止本 generation，随后重新走 snapshot/barrier；任何事件都不能静默丢弃。`inbox()` 不是 Relay 独立目录，而是由已验证的 Catalog/Conversation reducers 从 canonical pending approvals/terminal errors 派生。
...
- prompt 先显示 sending；daemon Accepted 后显示 queued；canonical UserMessage event 以 commandId 替换临时行；
  canonical terminal 也必须 commandId 精确匹配，上一 command 的 terminal 不得消费下一 prompt receipt。
- commandless Error 保持 streaming；command-bound Failed 收口当前 command，并允许下一 prompt 开始新 turn。
  snapshot direct Failed 的 turnID 可为空，此时必须以全局唯一 commandID 和 terminal-only approval 证据归并。
...
- P4.6 macOS persistent 远程 CLI automatic Task 已完成；production 使用 Keychain 中的真实 grant/private keys
  ...
  hardening 本身不新增 Task 完成数；在该 hardening/P4.6 收口时 P4 仍为 **6/7**。后续 P4.7 automatic
  E2E、静态 real-slot contract 与阶段文档已完成，focused `p4-auto`、独立顶层门禁、冻结 hash 与双路
  phase review 均通过，P4 automatic scope 现为 **7/7**，P4 automatic Phase Exit complete。
...
restart 必须读回 DeviceSign/DeviceHPKE/grant/counter/replay state，旧 credential JSON 不含 secret。Linux 只
允许 ephemeral test keys；macOS production persistent pairing 必须使用 Data Protection Keychain，
unsigned/ad-hoc 或 injected/file/dev keystore 降级均 typed unsupported。MVP 主线只运行 focused
`p4-auto`；远端 cannot-confirm pairing 由独立 RuntimeCore principal gate 证明，不在 machine E2E 内。
该 aggregate 不含顶层 `cargo test`、`swift test`、最终 diff/status、冻结 hash 或双路 phase review。
real runner 本期只是静态 versioned slot sentinel：不读取参数或环境变量，不探测 prerequisite、不执行真实
链路，固定输出完整 `missingInputs` 与 `BLOCKED/mutations=0/evidence=[]/summaryGenerated=false`；真实
preflight/execution 留给 post-MVP。MachineRoot 丢失仍按
[`docs/RELAY_RUNBOOK.md`](../RELAY_RUNBOOK.md#machineroot-丢失后的-portable-purge-receipt) 的 portable
purge receipt 流程处理。真实 vendor login、公网 WSS、provisioned signed
