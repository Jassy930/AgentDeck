...
overall closeout，W2 automatic overall complete。W3.1/W3.2 两类 deterministic crash cut、W3.3 第二
tab/Web Lock/generation 隔离、W3.4 真实 browser kill、W3.5 字节透明网络故障与 W3.6 同一 candidate 三次
fresh run均已关闭；W3.7 对完整 W3 增量完成 `spec/security` 与 `quality/Git` 双路终审，两路均
Approved，P0/P1/P2=0。Web Test Companion 因此达到 `Implemented (Web automatic test scope)`。
