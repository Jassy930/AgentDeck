...
> 2026-08-17 收敛说明：本文是旧 AppKit 路线的历史设计。N6 的 remote-ready
> `Transport` 预留已从主线删除；当前实现和下一步只以 GPUI reset 设计为准。
...
5. **AgentDeck 自带能力**：Project/Task/Run 工作台、Skill 管理、插件系统、SSH 远程、移动伴侣（长期路线图；不属于本设计的当前实现）
...
| v0.5 | daemon 远程化 + Web UI v0.1（历史设想，已从当前主线移除） | 6–10 周 |
