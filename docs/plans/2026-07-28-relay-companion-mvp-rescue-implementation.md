...
### R4.8/R4.9 restart handoff committed candidate 收口（2026-07-30）
...
- 三次 fresh lifecycle 连续 `3/3 PASS`。每轮均读回 daemon generation `2`、restart marker、history recovery、
  command `1/1`、approval `1/1`、revoke `1`、active grant `0`、Relay plaintext absent，以及
  host/root/invite/socket/owned Simulator cleanup 全部 absent。
- 同一 candidate 的完整 `p5` exit 0：SessionSource `25/25`；RelayClient
  `457 executed / 4 external entitlement skipped / 0 failed`。随后 `p4-auto` exit 0：machine E2E
...
- `spec/security` review 为 Approved，`P0/P1/P2=0`：确认 final Close 只在 Relay Core disconnect 与
  dependent-device cleanup 后返回；同进程 reconnect 保持原路径；Active startup reservation 只开放既有 512 条 /
  16 MiB 有界传输队列，不开放 Runtime，Dormant/Retire、未领取 lane、reservation drop 与 rollback 继续
  fail-close；协议/schema/storage/version 与 vendor-token 边界均未改变。
- `quality/Git` review 为 Approved，`P0/P1/P2=0`：健康 Close、device generation cut、reservation 首帧/drop 与
  manager rollback 均有确定性回归；candidate 只比失效的 `cfa90b6` 增加 5 个 code/test 路径和 2 个事实源文档，
  无冲突标记、二进制、DB、日志、secret 或运行数据进入 Git。
- ignored 的 `ios/AgentDeckMobile.xcodeproj/` 与 `ios/AgentDeckMobile/Info.plist` 已移动到本机可恢复废纸篓；
  `.build`、`target`、`dist` 缓存未删除。最终读回无 Relay/daemon/runner/xcodebuild 进程、无 Booted Simulator，
  HEAD/tree 未漂移且 `git status --short --branch` clean。该 candidate 当时关闭了 R4、P5.9 与 P5 automatic
  Phase Exit；后续 R5.2 的反例已使本节 phase 结论失效，具体以紧随其后的重开记录为准。

### R5.2 dual-scope readiness 反例与第七次 replacement R4.7（2026-07-30）

- R5 closeout candidate 为 `2ddba9ba54820c37fce7efe698cd58d869be2314`，tree 为
  `1f7e95f5d7543bebdd9e7f27513abc48cb9c0c8c`。同一 candidate 的完整 Rust workspace 为 daemon
  `1719 passed / 3 ignored / 0 failed`，其余 integration/doctest 零失败；随后完整 Swift 在
  `MachineScopeRealIntegrationTests` 稳定失败，focused 重跑仍可复现，host 30 秒内无法满足硬编码的
  `BusinessReady`。因此 Rust PASS 只保留为该失效 candidate 的诊断证据，不得拼接到 replacement candidate。
- 根因不是新本地运行平台、协议或 production runtime，而是同一个测试 host readiness 条件错误复用于两种合法
  拓扑。单远端/Simulator 的稳定资源形态必须精确为 writer/live/job `2/2/2`；macOS dual-scope 还由 registry 持有
  本机 source，稳定形态为 writer 精确 `3`、live/job 相等且为 `3` 或 `4`，barrier/snapshot sender 均为 `0`。
  Swift 已独立读回 remote catalog 与 conversation connected，host 仍继续验证 transition complete、active grant、
  semantic ACK 与资源残留，未放宽真实业务闭环。
- `8cfdc0d` 的旧 R4 evidence 与 `2ddba9b` 的 R5 closeout candidate 均已失效；旧三次 lifecycle、aggregate、
  全量与 review 结果不能作为当前 R4/R5 完成证据。R4 回到 replacement R4.7，R5 等待 R4.8/R4.9 再次闭合后
  才能重新执行 R5.1–R5.6。
- replacement implementation commit 为 `6dc2822942a418e144d38735a0a51892cf7a885d`，tree 为
  `b887255f2a45d44f0eee1b2cf5b2e13b071828cc`，相对本地 `master` 为 `0 behind / 300 ahead`。本切片只修改
  `agentdeckd/tests/relay_v2_machine_e2e.rs` 与
  `Tests/AgentDeckTests/MachineScopeRealIntegrationTests.swift`，2-path content manifest SHA-256 为
  `e8a8ea64101bb509dd55a61005296a48004e34cd12894a19403cc2e1a67993ec`；未修改 production、协议、schema、
  storage 或版本。
- fresh implementation gates：machine E2E `3 passed / 1 interactive ignored / 0 failed`；dual-scope focused 连续
  `3/3 PASS`；完整 Swift `1169 XCTest / 4 external entitlement skipped / 0 failure + 48 Swift Testing PASS`；
  Rust/Swift format、`git diff --check` 与 scoped Clippy 均 PASS。直接全局 `-D warnings` 只命中三项既有 Codex
  adapter lint；按 `docs/QUALITY.md` 冻结 allow 列表重跑通过，没有修改无关代码。
- 本节与 `docs/QUALITY.md` 以 docs-only scoped commit 生成第七次 replacement R4.7 candidate。提交后必须读回
  commit/tree/clean status，并从零连续执行三次 fresh lifecycle、完整 `p5`、`p4-auto`、必要的完整
...
- [x] R4.8：在 replacement committed candidate 上从零连续执行三次 fresh E2E。
- [x] R4.9：同一 replacement candidate 执行完整 `p5`/`p4-auto`、双路 review、cleanup 与 clean status；若修改
  代码，从 R4.7 重来。
...
### R5.1 restart handoff closeout candidate（2026-07-30）

> 已失效：本节生成的 `2ddba9b` candidate 在 R5.2 完整 Swift 暴露 dual-scope readiness 断言错误；当前必须先
> 完成第七次 replacement R4.7–R4.9，再重新执行 R5.1。

- 本 Task 只更新本计划与 `docs/QUALITY.md`，把 replacement R4.8/R4.9 的重复性、`p5`/`p4-auto`、双路 review
  和 cleanup 写回事实源；不修改 production、test、runner、协议、schema、storage 或版本。
- R4 evidence candidate 固定为 `8cfdc0d0bfd74186dafaf09da4333f97e10803a2` / tree
  `dca2aa3c1a5da8c08bdd9f2b1eff83c825f754e9`。包含本节的 docs-only scoped commit 生成新的最终 closeout
  candidate；R5.2 必须从提交后 clean 状态重新读回 commit/tree/path hashes，并把完整 Rust/Swift/iOS/P4/P5、
