...
Swift UI、旧 daemon transport 与远程路线。当前产品决策是停止在旧
...
- 不实现远程机器、配对、历史、审批、Markdown 或多窗口。
- 不为旧 Swift 桌面接口建立兼容层。
...
禁止把 `agentdeckd` 链接进 GUI 进程；禁止 UI 直接解析 vendor JSON。
...
- selfcheck 只报告真实初始化的 `status`、`surface` 与 `ui`，不为未实现能力设置占位字段。
