...
- Thread 行展示项目名、标题或 preview、更新时间、状态，并用透明背景小图标标记
  agent 来源；当前 Codex 历史会话显示本地打包的 Codex SVG，后续 adapter 复用同一映射入口。
...
- Thread 详情右侧显示无背板的竖排轮次 rail：每个用户消息一枚小点，hover 时临时放大并展示摘要，
  点击跳转到该轮；点位固定间距并居中，溢出时 rail 跟随当前轮次自动揭示点位；最底部固定“最新”点，
  点击跳到会话底部，rail 区域滚轮按轮次跳转且边界不循环；用户手动滚动会话流时，
  rail 高亮被动跟随当前阅读位置。
