...
//   └─────────────────────────────────────────────────────────┘
//
// Backed by a canonical `PendingActionRequest` (ThreadRuntimeModel.swift).
// Approve/Deny 回传当前卡片实际渲染的 exact binding，不重新读取“第一条 pending”。
@MainActor
final class ApprovalCardView: NSView {

  private weak var model: SessionModel?
  private var action: PendingActionRequest?
  ...
  @objc private func approve() {
    ...
    model?.decidePendingAction(action, decision: .approve, persist: persist)
  }

  @objc private func deny() {
    guard let action else { return }
    model?.decidePendingAction(action, decision: .deny)
  }
}
