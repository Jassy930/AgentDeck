...
//
// 提交时调用 `Self.buildSessionStart(...)` 装配 `SessionStart`，通过
// `onSubmit` 回调交回上层（通常由 SessionViewController 转发给
// `WorkbenchModel` / `DaemonClient`）。
@MainActor
public final class NewSessionDialog: NSWindowController {

    public var onSubmit: ((SessionStart) -> Void)?
    ...
    private func installVendorForm(for kind: AgentKind) {
        vendorFormContainer.subviews.forEach { $0.removeFromSuperview() }
    ...
    @objc private func submit() {
        ...
        let start = Self.buildSessionStart(
    ...
    /// 纯函数，便于在测试中校验 SessionStart 装配。
    public static func buildSessionStart(
        agentKind: AgentKind,
