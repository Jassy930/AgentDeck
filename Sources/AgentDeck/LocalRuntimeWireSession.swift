...
/// App model 使用的本机 Runtime v2 wire 边界。它只在已经完成安全 UDS/Hello 的
...
actor LocalRuntimeWireSession {
  ...
  private static func invalidTransferPayloadFailure() -> RuntimeEnvelopeClientFailure {
    RuntimeEnvelopeClientFailure(
      ...
      message: "completed transfer is not one exact current Runtime v2 payload"
    )
  }
}
...
/// current Runtime v2 DTO canonical encode 后的裸 payload。这里只接受当前 production
