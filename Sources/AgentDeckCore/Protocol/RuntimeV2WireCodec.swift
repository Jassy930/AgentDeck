...
extension RuntimeSyncCompleteV1 {
  fileprivate init(runtimeV2FlattenedFrom decoder: Decoder) throws {
    try runtimeV2RejectUnknownKeys(
      decoder,
      allowed: [
...
public enum RuntimeV2WireCodec {
  ...
  /// compact ADRT1 已完成 64 MiB/hash/budget gate 后的唯一大 payload decode。
  /// 普通 JSON/UDS 入口仍严格保持 1 MiB；这里仅允许与 carrier channel 一致的
  /// Reply/Stream，拒绝 Request 与嵌套 TransferPart。
  public static func decodeAssembledTransferEnvelope(
    _ data: Data,
    channel: RuntimeTransferChannelV2
  ) throws -> RuntimeEnvelopeV2 {
    guard !data.isEmpty, UInt64(data.count) <= TransferEnvelopeV2.maxTotalBytes else {
      throw RuntimeV2WireError.transferTooLarge
    }
    let value = try JSONDecoder().decode(RuntimeEnvelopeV2.self, from: data)
    switch (channel, value.body) {
    case (.reply, .reply(let reply)):
      guard case .transferPart = reply else { return value }
    case (.stream, .stream(let item)):
      guard case .transferPart = item else { return value }
    case (.reply, .request), (.reply, .stream), (.stream, .request), (.stream, .reply):
      break
    }
