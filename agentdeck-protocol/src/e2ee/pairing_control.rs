...
//!
//! PairPending 由 MachineDataSign 签名；PairResponseReceived 由 DeviceSign 签名。两者都
//! 绑定完整 pair trust domain，且不复用要求非空 pre-grant machine route 的通用 TBS。
...
use crate::e2ee::pairing::{
    ...
    PairPendingV1, PairRequestInfoV1, PairResponseInfoV1, PairResponseReceivedV1, PairingError,
};
...
/// PairPending 等已签名 pairing control plaintext 的 HPKE carrier。Relay 只能看到
/// `enc+ciphertext`，看不到 requestHash 或 control subtype。
