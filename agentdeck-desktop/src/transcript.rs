...
use gpui::{App, IntoElement, ParentElement, div, prelude::*, px};
...
pub fn render(turns: &[HistoryTurn], cx: &App) -> impl IntoElement + use<> {
    let blocks: Vec<_> = turns
        .iter()
        ...
        .filter_map(|item| {
            ...
            Some(
                v_flex()
                    ...
                    .child(div().text_sm().child(truncate(&body))),
            )
        })
        .collect();
...
#[cfg(test)]
mod tests {
    use super::{BODY_LIMIT, describe, truncate};
    use agentdeck_protocol::{AgentItem, AgentItemMeta, ShellStatus};
