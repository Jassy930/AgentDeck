...
- **接入本机 `agentdeckd` 的只读历史**：启动时先问 daemon 注册了哪些 agent，再按
  ...
  工具、变更默认折叠为一行摘要（命令附状态、退出码与耗时，失败标红），点击展开；
  工具结果中的 base64 大串显示为“二进制内容 N KB”。加载中、失败和空结果都有明确文案；某个来源失败时保留其他来源的会话，并在侧栏显示该来源的错误。
...
历史 warning 包含实际版本、路径和已验证版本；桌面在来源与正文区域显示，CLI 输出到
