...
- **接入本机 `agentdeckd` 的只读历史**：启动时先问 daemon 注册了哪些 agent，再按
  agent 分别拉取会话列表（首批各显示 50 条），谁先返回谁先进侧栏；列表末尾的
  ...
  的会话，侧栏 agent 行显示“失败”与重试按钮，错误原因在悬停详情中。
  ...
  时:分，更早为月/日）。每条会话带 16px 彩色来源像素图标。
...
- 侧栏搜索框按会话标题或项目名（不区分大小写）实时过滤列表；点击空态的 agent 卡片
  或侧栏底部的 agent 行只看该 agent 的会话，再点取消。两种过滤可叠加，无结果时显示
  “没有匹配的会话”。
- 侧栏底部每个 agent 一行：图标、名称、已加载数量；有兼容性警告时显示 ⚠。完整状态、
  警告和错误原因放在悬停详情里。
