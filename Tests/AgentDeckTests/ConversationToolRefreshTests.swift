...
final class ConversationToolRefreshTests: XCTestCase {
    ...
    func testStableToolCallRowReconfiguresFromRunningToTerminalState() throws {
        ...
        model.replaceFixtureItems([user, running])
        ...
        model.replaceFixtureItems([user, completed])
    ...
    func testCollaborationActivityRefreshesItsEventDescriptionInPlace() throws {
        ...
        model.replaceFixtureItems([user, started])
        ...
        model.replaceFixtureItems([user, interacted])
        RunLoop.current.run(until: Date(timeIntervalSinceNow: 0.1))
    ...
    func testRepeatedCollaborationActivityRendersOneExpandableProcessGroup() throws {
        ...
        interacted.activityEvent = "interacted"
        model.replaceFixtureItems([user, started, reasoning, interacted])
    ...
    func testStableCollapsedGroupRefreshesCountAndFailureThenKeepsExpansionOnAppend() throws {
        ...
        model.replaceFixtureItems([user, first, second])
        ...
        model.replaceFixtureItems([user, first, second, failed])
        RunLoop.current.run(until: Date(timeIntervalSinceNow: 0.1))
        ...
        model.replaceFixtureItems([user, first, second, failed, fourth])
        RunLoop.current.run(until: Date(timeIntervalSinceNow: 0.1))
