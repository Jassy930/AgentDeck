...
- **`tokens.css`** — L2 皮肤层。四套主题通过 `[data-theme]` 声明颜色/字体/圆角/阴影/动效（纯值）。
...
- **[`tokens/tokens.json`](tokens/tokens.json)** — **单一数据源（SSOT）**：6 套主题的色板/圆角/字体/阴影/结构枚举 + 平台开关。改这里，其余生成。
- **`generated/`** — 生成物（**禁手改**）：`Theme.swift`（AppKit：`Palette`/`Radii`/`Structure` 枚举/`Platform`，视图读契约、禁主题分支）、`tokens.css`、`DesignTokens.ts`。
...
3. **组件库** — 按钮 / 图标按钮、徽章标签（权限 / Plan Mode / effort / 队列）、运行状态点 + 加载环、Composer；**表单控件**（开关 / 分段 / 芯片 / 输入 / 头像 / 用量条）；**导航与菜单**（Tab bar / 上下文菜单）；**覆盖层与反馈**（Toast / 对话框 / 底部弹层 / 空状态）；**卡片**（审批 / 连接器 / 环境面板）；**内容展示**（消息气泡 / 命令块 / Diff 行 / 设置行 / 图标块 / kbd）；列表行（历史线程 / 项目组）。
4. **桌面工作台** — 透明标题栏 + 全高侧栏 + 流式会话 + 悬浮 composer + 环境面板 + 轮次导航；真实尺寸 1280×820 可缩放预览；点击侧栏身份打开**设置窗口**（分类导航 + 身份/主机/Agent/… 内容面板）。
