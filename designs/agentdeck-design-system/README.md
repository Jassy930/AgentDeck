...
5. **移动端** — 以 iPhone 390×844pt 真机为基准；**三屏 · 列表↔详情导航**：会话列表（机器 + 项目平铺分组，标明 fixture 数据来源）、会话页（输入消息并查看 fixture 回放；流内审批卡、去 tab、返回列表、最大化工作区）、我的（身份标识 / fixture 数据概览 / GitHub 式用量热力图→详情）。触控目标 ≥ 44pt。**iOS 与 Android 两版**：同一套内容组件，仅设备 chrome 不同（刘海+home indicator ↔ punch-hole+手势条），由 `data-platform` 平台轴驱动。
