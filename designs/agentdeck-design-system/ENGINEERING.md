...
> 当前消费方为 Web / RN、iOS，以及使用 codex 颜色的 GPUI 桌面端。
> GPUI 的接入范围是颜色生成物与 gpui-component 字段映射。
>
> 本文件是**具有约束力**的工程契约。凡消费本设计系统的代码（Web / iOS / GPUI / 其它），
...
| **L1 生成物（禁手改）** | `generated/tokens.css`·`generated/DesignTokens.ts`·`ios/AgentDeckMobile/DesignTokens.swift`·`agentdeck-desktop/src/theme_tokens.rs` | 生成器 | Web / RN / iOS / GPUI |
...
### MUST（必须）
- **M1** 一切颜色/圆角/字体族/字号/行高/间距/阴影，**只能引用 token**（Web：`var(--x)`；iOS：生成的 `DesignTokens`）。GPUI 当前接入的颜色由生成的 `theme_tokens.rs` 提供，再通过 `theme.rs` 映射到组件库；其它 token 在对应切片接入，不允许恢复 Swift macOS 生成物。
...
- **N3** ❌ 手改 Web、iOS 或 GPUI 生成物，或让 `tokens.css` 与 SSOT 漂移；Rust 颜色漂移由 `--check-desktop` 检查。
...
bun run build          # → Web / RN、iOS 与 GPUI 颜色生成物
bun run check          # build + lint + a11y
node tools/build.mjs --check-desktop  # 只读核对 Rust 生成物，漂移时失败
...
4. `build.mjs --check-desktop` — 比对 SSOT 应生成的 Rust 文本与已提交的 `theme_tokens.rs`，不写文件；仓库 CI 从根目录运行 `node designs/agentdeck-design-system/tools/build.mjs --check-desktop`。
...
- GPUI 使用 `agentdeck-desktop/src/theme_tokens.rs` 中当前消费的 codex `Rgba` 颜色常量；该文件由 `tools/build.mjs` 生成，改颜色只改 SSOT 并重新生成。
- `agentdeck-desktop/src/theme.rs` 在组件库初始化后映射字段并固定为深色，当前不提供主题切换入口。GPUI 字体、圆角、排版等尚未通过该生成物接入。
- codex 的 `warn` / `warnWeak` 在 SSOT 统一为琥珀 `#f5b544` / 透明度 `0.14`，同步 Web 与 iOS；GPUI 选区使用 `accentWeak`，保留其 `0.14` 透明度。
- primary、secondary、warning 的交互状态从基础 token 通过 `lighten` / `darken` 派生。ghost 的 hover / pressed 使用组件库对 `secondary` 的派生，选中态使用 `secondary_active`。
...
- ✅ 已工程化：SSOT + Web/RN/iOS 生成器 + GPUI 颜色生成物 + 门禁 + 无障碍核验 + 契约文档。
- 🚧 当前 GPUI 仅消费 codex 颜色并固定为深色；其它主题与非颜色 token 尚未接入。
